...
func test_parse_direction_abbreviation_n() -> void:
	var result: Dictionary = fmt.parse_response('{"direction": "N"}')
	assert_eq(result["type"], "DIRECTION")
	assert_eq(result["dir"], Enums.MoveDirection.NORTH)


func test_parse_direction_alias_up() -> void:
	var result: Dictionary = fmt.parse_response('{"direction": "UP"}')
	assert_eq(result["type"], "DIRECTION")
	assert_eq(result["dir"], Enums.MoveDirection.NORTH)


func test_parse_direction_alias_right() -> void:
	var result: Dictionary = fmt.parse_response('{"direction": "RIGHT"}')
	assert_eq(result["type"], "DIRECTION")
	assert_eq(result["dir"], Enums.MoveDirection.EAST)


func test_parse_direction_alias_down() -> void:
	var result: Dictionary = fmt.parse_response('{"direction": "DOWN"}')
	assert_eq(result["type"], "DIRECTION")
	assert_eq(result["dir"], Enums.MoveDirection.SOUTH)


func test_parse_direction_alias_left() -> void:
	var result: Dictionary = fmt.parse_response('{"direction": "LEFT"}')
	assert_eq(result["type"], "DIRECTION")
	assert_eq(result["dir"], Enums.MoveDirection.WEST)


func test_parse_json_with_surrounding_text() -> void:
	var result: Dictionary = fmt.parse_response('I think north. {"target": [3, 2]}')
	assert_eq(result["type"], "TARGET")
	assert_eq(result["pos"], Vector2i(3, 2))


func test_parse_empty_string() -> void:
	var result: Dictionary = fmt.parse_response("")
	assert_eq(result["type"], "NONE")


func test_parse_invalid_direction() -> void:
	var result: Dictionary = fmt.parse_response('{"direction": "NORTHEAST"}')
	assert_eq(result["type"], "NONE")


func test_parse_missing_fields() -> void:
	var result: Dictionary = fmt.parse_response('{"foo": "bar"}')
	assert_eq(result["type"], "NONE")


func test_parse_no_json() -> void:
	var result: Dictionary = fmt.parse_response("not json at all")
	assert_eq(result["type"], "NONE")


func test_parse_invalid_target_format() -> void:
	var result: Dictionary = fmt.parse_response('{"target": "invalid"}')
	assert_eq(result["type"], "NONE")


func test_parse_target_wrong_array_size() -> void:
	var result: Dictionary = fmt.parse_response('{"target": [1]}')
	assert_eq(result["type"], "NONE")


func test_parse_invalid_target_falls_back_to_direction() -> void:
	var result: Dictionary = fmt.parse_response('{"target": "bad", "direction": "SOUTH"}')
	assert_eq(result["type"], "DIRECTION")
	assert_eq(result["dir"], Enums.MoveDirection.SOUTH)
...
func test_build_system_message_contains_rules() -> void:
	var msg: String = fmt.build_system_message("Go north always", 3)
...
func test_build_state_message_contains_turn() -> void:
	_setup_game()
	var msg: String = fmt.build_state_message(0, maze, fog_node, gm, kc, wc, 42)
...
func test_inactive_key_not_in_prompt() -> void:
	...
	_assert_not_contains(msg, "[KEY:JADE]")
	_assert_not_contains(msg, "[KEY:CRYSTAL]")


func test_inactive_chest_not_in_prompt() -> void:
	_setup_game()
	var msg: String = fmt.build_state_message(0, maze, fog_node, gm, kc, wc, 1)
	_assert_not_contains(msg, "[CHEST]")


func test_token_estimate() -> void:
	var estimate: int = fmt.get_token_estimate("Hello world this is a test")
...
func _assert_not_contains(text: String, substring: String) -> void:
