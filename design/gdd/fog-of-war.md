...
4. **视野更新时机**：每当 Agent 移动到新 cell 时触发一次视野重算。Agent 不移动则视野不变。**初始视野刷新**：`FogOfWar.initialize(maze, agent_ids)` 在为每个 Agent 创建 VisionMap 后，**自动从 MazeData 读取 `SPAWN_A` / `SPAWN_B` marker 位置**，并对每个 Agent 立即调用一次 `update_vision(agent_id, spawn_position)`，确保比赛开始时 Agent 的出生点周围已有视野。此初始调用不依赖 Grid Movement 的 `mover_moved` 信号。Spawn 位置的唯一真相源是 MazeData 的 spawn marker——与 Grid Movement 的 `initialize()` 使用同一数据来源，避免分叉
...
FogOfWar:
  ...
  # --- 对外接口 ---
  initialize(maze: MazeData, agent_ids: Array<int>)   # 创建所有 Agent 的 VisionMap，从 MazeData 读取 SPAWN_A/B 位置，并对每个 Agent 立即调用 update_vision(agent_id, spawn_pos) 刷新初始视野
...
- **生命周期契约——总是 `initialize()`**：
  - **新比赛（新迷宫）或 Rematch（同迷宫）**：统一调用 `initialize(maze, agent_ids)`。`initialize()` 内部会：1) 创建或重建 VisionMap（匹配迷宫尺寸），2) 绑定 MazeData 引用，3) 从 MazeData 读取 spawn 位置刷新初始视野。Rematch 时传入同一 MazeData 即可，`initialize()` 会重置旧状态
  - `_reset(agent_id)` 和 `_reset_all()` 是**内部工具方法**，供 `initialize()` 内部调用。外部系统不应直接调用它们——始终通过 `initialize()` 完成生命周期管理
  - **禁止**：仅调用 `_reset_all()` 而不 `initialize()`——会导致初始视野未刷新（首帧全黑）或 MazeData 引用过期
...
| **Match State Manager** | MSM → FoW | `initialize()` | COUNTDOWN 阶段，FoW 监听 `state_changed` 信号调用 `initialize(maze, agent_ids)`。新比赛和 rematch 均走此路径——`initialize()` 内部处理重置和初始视野刷新 |
...
| Agent 位置就是 Spawn 点（比赛刚开始） | `FogOfWar.initialize()` 内部从 MazeData 读取 `SPAWN_A`/`SPAWN_B` 位置，自动对每个 Agent 调用 `update_vision(agent_id, spawn_pos)`，Spawn 周围 vision_radius 范围内的 cell 变为 Visible，其余保持 Unknown。不依赖 Grid Movement 的 `mover_moved` 信号 | 比赛开始时 Agent 不应该处于完全黑暗状态——初始位置周围的视野是必要的起步信息。Spawn 位置从 MazeData 读取，与 Grid Movement 使用同一真相源 |
...
| Match 重新开始（rematch，同一迷宫） | 调用 `initialize(same_maze, agent_ids)`，与新比赛流程相同。`initialize()` 内部重置所有 VisionMap 并刷新初始视野。外部系统不需要单独调用 `_reset_all()` | 统一生命周期：无论 rematch 还是新迷宫，外部系统只需调用 `initialize()`。简化调用方责任 |
| Match 使用新迷宫开始（new match） | 调用 `initialize(new_maze, agent_ids)`。`initialize()` 会：1) 创建新的 VisionMap（匹配新迷宫尺寸），2) 绑定新 MazeData 引用，3) 从新 MazeData 读取 spawn 位置，4) 刷新初始视野 | 新迷宫的尺寸、spawn 位置、结构可能与旧迷宫完全不同。`initialize()` 内部处理所有重建逻辑 |
...
### 核心功能
- [ ] `FogOfWar.initialize(maze, [0, 1])` 后，Spawn 位置（从 MazeData 的 `SPAWN_A`/`SPAWN_B` 读取）及 BFS vision_radius 范围内的 cell 为 `VISIBLE`（初始视野已刷新），范围外为 `UNKNOWN`
- [ ] `initialize()` 从 MazeData 读取 spawn 位置（与 Grid Movement 使用同一真相源），内部自动调用 `update_vision()` 完成初始视野刷新——不依赖外部 `mover_moved` 信号，也不依赖外部传入 spawn 坐标
...
### 生命周期
- [ ] Rematch 时再次调用 `initialize(same_maze, agent_ids)` 等效于对同一迷宫重新开始，初始视野正确刷新，旧 VisionMap 状态完全清除
