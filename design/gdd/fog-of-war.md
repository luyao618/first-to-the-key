...
7. **寻路与信息边界**：LLM Agent Integration 的 A* 寻路在完整迷宫上运行（不限制在已知区域），但 LLM 的**目标选择**严格限制在 visible/explored 区域、LLM 的**输入信息**严格遵循 FoW 三态规则。这是一个有意的设计取舍——详见 `llm-agent-integration.md` 的"A* 寻路范围决策"说明
