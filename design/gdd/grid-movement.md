...
5. 外部系统（LLM Agent Integration / 玩家输入）在任意时刻写入 `pending_direction`，Grid Movement 在下一个 tick 消费它。如果一个 tick 内被多次写入，以最后一次为准
6. 两个 Mover 可以占据同一 cell，不产生碰撞或交互
7. Mover 初始位置从 MazeData 的 `SPAWN_A` / `SPAWN_B` 标记读取。`initialize(maze)` 由 Grid Movement 监听 Match State Manager 的 `state_changed(_, COUNTDOWN)` 信号时自动调用，确保 Agent 在倒计时阶段已就位但不可移动（tick 仅在 PLAYING 状态触发）
...
Mover:
  ...
  pending_direction: MoveDirection  # 下一 tick 要执行的方向（NONE = 不动）
...
GridMovementManager:
  ...
  # --- 外部输入接口 ---
  set_direction(mover_id, direction: MoveDirection)  # 设置下一 tick 的移动方向
  ...
  # --- 生命周期 ---
  initialize(maze: MazeData)       # 读取 spawn 点，创建 Mover，记录初始位置到 visited_cells
  reset()                          # 清空所有 Mover 数据
```
...
on_tick(tick_count):
  for mover in movers:
    ...
    if dir == NONE:
      emit mover_stayed(mover.id, mover.position)
      continue
    ...
    if maze.can_move(mover.position.x, mover.position.y, dir):
      ...
      emit mover_moved(mover.id, old_pos, mover.position)
    else:
      mover.blocked_count += 1
      emit mover_blocked(mover.id, mover.position, dir)
```
...
| **Fog of War** | FoW -> Movement | 监听 `mover_moved` 信号，查询 `get_position()` | 移动后更新可见区域 |
...
| total_reachable_cells | int | 1 to width * height | MazeData（连通 cell 总数） | 迷宫中可达的 cell 总数 |
...
| Variable | Type | Range | Source | Description |
|----------|------|-------|--------|-------------|
| optimal_path_length | int | 1+ | `MazeData.get_shortest_path()` 长度 | 起点到目标的最短路径步数 |
| total_moves | int | 1+ | Mover.total_moves | 实际成功移动的总步数 |
| efficiency | float | 0.0 to 1.0 | 计算结果 | 1.0 = 完美路径，越低越多冗余移动 |

注意：efficiency > 1.0 理论上不可能（除非目标在移动过程中改变）。efficiency 在 Agent 还未到达目标时无意义，仅用于赛后统计。
...
| `initialize()` 时 MazeData 中缺少 SPAWN_A 或 SPAWN_B | 打印错误日志，将缺失的 Mover 位置设为 `(0, 0)` | 不崩溃，但这本质是 Maze Generator 的 bug（`is_valid()` 应已拦截） |
...
| 超大迷宫（50x50）中 tick 处理时间 | 每 tick 处理 2 个 Mover，每个仅 1 次 `can_move()` 调用 + 1 次数组查找，远低于 1ms | Grid Movement 的 tick 复杂度是 O(n)，n = Mover 数量（MVP 中 n=2），与迷宫大小无关 |
...
| **Match State Manager** | Grid Movement depends on this | 监听 `tick` 信号驱动移动处理，监听 `state_changed(_, COUNTDOWN)` 信号触发 `initialize(maze)` 完成 Mover 就位，监听 `state_changed` 信号管理生命周期（PLAYING 执行、FINISHED 停止） |
...
| **Fog of War / Vision** | FoW depends on this | 监听 `mover_moved` 信号更新可见区域，查询 `get_position()` 和 `get_visited_cells()` |
...
- [ ] `initialize(maze)` 后，Mover A 的位置等于 `MazeData.get_marker_position(SPAWN_A)`，Mover B 同理
- [ ] `initialize(maze)` 后，两个 Mover 的 `visited_cells` 各包含自己的起始位置
- [ ] `set_direction(0, EAST)` 后，下一个 tick Mover 0 向东移动一格（若可通行）
