...
KeyCollectionManager:
  ...
  # --- 生命周期 ---
  initialize(maze: MazeData)       # 读取钥匙位置，初始化状态（COUNTDOWN 时调用）
  reset()                          # 清空所有状态
```
...
| **Match State Manager** | Keys → MSM | 监听 `state_changed` 信号 | Key Collection 监听 MSM 的 `state_changed`：COUNTDOWN 时初始化状态，PLAYING 时启用拾取判定，FINISHED 后清理 |
...
| Agent 站在 Jade Key cell 上且 `next_key` 为 `KEY_JADE`，但 Jade 尚未被激活（仍在 BRASS_ACTIVE 阶段） | 不拾取。钥匙未 Active，判定条件不满足 | Inactive 钥匙对拾取逻辑不可见。Agent 需等待全局阶段推进到 JADE_ACTIVE |
...
| Agent 被 `initialize()` 放置在 Spawn 点，而 Spawn 点恰好就是 Brass Key 所在 cell | 不自动拾取。拾取仅由 `mover_moved` 信号触发，`initialize()` 不触发 `mover_moved` | 拾取判定严格绑定移动事件，初始放置不算移动。如果确实重合，Agent 第一次移动离开再回来（或原地不动一个 tick）都不会触发拾取——只有 `mover_moved`（成功移动到新位置）才触发 |
| 所有钥匙放置在同一个 cell 上 | 理论上合法但游戏体验极差。Agent 到达该 cell 时，第一次 `mover_moved` 只拾取 Brass（`next_key` 匹配），Jade 此时变为 Active——但 Agent 已经在该 cell 上了，不会再触发 `mover_moved`。Agent 需要离开再回来才能拾取 Jade，再离开再回来拾取 Crystal | 拾取判定仅在 `mover_moved` 时触发，不会在 `key_activated` 时重新检查当前位置。这是有意的简化——Maze Generator 应保证钥匙分散放置，不会出现重合 |
...
| AC-11 | Inactive 钥匙不可拾取 | 单元测试：Jade 为 Inactive 时，Agent（`next_key` 为 `KEY_JADE`）站在 Jade cell 上，断言不触发拾取 |
...
| # | Criterion | Verification |
|---|-----------|-------------|
| AC-16 | `initialize(maze)` 正确读取 MazeData 中的 3 个钥匙位置 | 单元测试：构造含 3 个钥匙 marker 的 MazeData，`initialize()` 后断言内部缓存的钥匙位置正确 |
| AC-17 | `reset()` 清空所有状态（全局阶段回到 BRASS_ACTIVE，所有 Agent 进度回到 NEED_BRASS） | 单元测试：完整比赛后调用 `reset()`，断言所有状态恢复初始值 |
...
|---|----------|--------|--------|
| OQ-1 | 钥匙激活时是否需要通知 LLM？当前设计中 LLM 通过 LLM Information Format 在每个 tick 获取完整状态快照，所以不需要单独推送"新钥匙出现了"的事件。但如果 LLM 没有被明确告知阶段变化，可能导致 prompt 策略难以写"当 Jade 出现时切换目标" | Medium | 推迟到 LLM Information Format 设计时决定 |
| OQ-2 | `chest_unlocked` 信号是否应携带宝箱位置？当前设计只传 `agent_id`，宝箱位置由 Win Condition / Chest 系统从 MazeData 读取。但如果宝箱位置是在 `chest_unlocked` 时才动态生成的（而非 Maze Generator 预设），则需要不同的数据流 | Medium | 推迟到 Win Condition / Chest 设计时决定 |
