...
3. Inactive 的钥匙对玩家不可见——消费方（Match Renderer / LLM Information Format）查询 `is_key_active()` 自行过滤，Renderer 不渲染、LLM 不报告。拾取判定也跳过 Inactive 钥匙。**注意：FoW 不负责此过滤**——FoW 仅管理 cell 三态可见性
