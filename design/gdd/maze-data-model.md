...
MazeData:
  ...
  reset()                                  # 将 Finalized 或 Uninitialized 状态回退为 Uninitialized：所有 cell 恢复为四面有墙、清除所有 markers、解锁写入接口。用于 Maze Generator 重试时复用同一实例

  # --- 验证接口 ---
  is_valid() -> bool                       # 检查迷宫完整性（边界封闭、必要标记存在、6 个标记位置互不重复、迷宫完全连通）
...
- **`reset()` 支持重试复用**：Maze Generator 在重试时调用 `reset()` 将同一实例恢复为全墙、清除 markers、回退到 Uninitialized 状态，避免每次重试创建新对象带来的 GC 压力（50 次重试 = 50 个临时对象）
- **`get_shortest_path` 在数据模型中**：BFS 仅需墙壁数据，是纯数据操作，且被多个系统需要（公平性验证、距离信息）
- **`cells[y][x]` 存储顺序**：行优先（先选行 y 再选列 x），但公开 API 统一用 `(x, y)` 参数顺序

### States and Transitions

Maze Data Model 本身是静态数据，但它有三个生命周期阶段：

| State | Entry Condition | Exit Condition | Behavior |
|-------|----------------|----------------|----------|
| **Uninitialized** | `MazeData` 对象刚创建，或 `reset()` 被调用 | Maze Generator 调用 `finalize()` | 所有 cell 初始化为四面有墙、无标记。写入接口可用，查询接口可用但返回的是"全墙"状态 |
| **Finalized** | `finalize()` 调用成功（`is_valid()` 通过） | `reset()` 被调用，或 Match 结束对象释放 | 写入接口被锁定（调用会产生错误日志但不崩溃）。所有查询接口正常工作。迷宫结构不可变 |
...
- **Uninitialized → Finalized**：`finalize()` 内部调用 `is_valid()`，验证通过后锁定写入接口
- **Finalized → Uninitialized**：`reset()` 恢复所有 cell 为四面有墙、清除所有 markers、解锁写入接口。用于 Maze Generator 重试时复用同一实例
- **Uninitialized → Uninitialized**：`reset()` 在 Uninitialized 状态也可调用（幂等），等效于重新初始化为全墙网格
...
| 同一 cell 放置多个标记：`place_marker(3, 3, KEY_BRASS)` + `place_marker(3, 3, SPAWN_A)` | 允许写入。一个 cell 可以同时持有多个不同类型的标记。但 `is_valid()` 会检测到标记位置重复并返回 `false`，`finalize()` 因此也会失败 | 写入接口不限制，验证接口兜底。Maze Generator 的放置规则（每个 marker 独占一个 cell）是业务约束，由 `is_valid()` 统一校验而非 `place_marker()` 逐一拦截 |
...
- [ ] `is_valid()` 在以下情况返回 `false`：缺少 SPAWN_A/B、缺少任意钥匙、缺少 CHEST、6 个标记中存在位置重复（任意两个标记在同一 cell）、迷宫不连通
- [ ] `is_valid()` 在所有标记齐全且迷宫完全连通时返回 `true`
- [ ] Finalized 后调用 `set_wall()` / `place_marker()` 不修改数据，并打印错误日志
- [ ] `reset()` 后所有 cell 恢复为四面有墙、所有 markers 清除、写入接口重新可用
- [ ] `reset()` 在 Uninitialized 状态调用不报错（幂等）
- [ ] `reset()` 后 `finalize()` 可再次成功调用（完整写入 + 验证流程可重复执行）
...
| `reset()` 是否需要添加到 MazeData 生命周期？ | Technical Director | Sprint 1 | **Resolved 2026-04-04**: 已添加。`reset()` 将 Finalized 或 Uninitialized 状态回退为 Uninitialized（全墙、无 markers、写入接口解锁）。Maze Generator 重试时调用 `reset()` 复用同一实例，避免 GC 压力。同步更新了生命周期状态图、Data Structures、Edge Cases 和 Acceptance Criteria |
| `is_valid()` 是否应检查 6 个标记位置互不重复？ | Technical Director | Sprint 1 | **Resolved 2026-04-04**: 已添加。`is_valid()` 现在检查所有 6 个标记（SPAWN_A/B、KEY_BRASS/JADE/CRYSTAL、CHEST）的位置互不重复。这使得 MazeData 成为"标记唯一性"的单一事实来源（Single Source of Truth），下游系统（Key Collection、Win Condition）可以安全假设标记不重叠 |
