...
**三栏容器所有权**：三栏容器（HBoxContainer，比例 20/60/20）是 **Match 场景的根级节点结构**，不属于任何单个 Presentation 系统。Prompt Input、Match Renderer、Match HUD、以及未来可能的 Match 内嵌结果 overlay 都作为子节点挂载到对应栏位下。各系统通过 Match State Manager 的 `state_changed` 信号协调自身内容的显示/隐藏，三栏容器本身始终存在。Match Renderer 的 God View 渲染在中栏的 SubViewportContainer 内，摄像机仅适配中栏可视区域（~60% 宽度 × 全高度）。
...
**职责边界**：Prompt Input 系统仅管理 SETUP 阶段左右栏的内容（prompt 输入 + 等待提示 + Ready 状态）。COUNTDOWN 及之后阶段的栏内容由各自负责的系统管理。三栏容器本身是 Match 场景的根级节点结构（见上方"三栏容器所有权"），Prompt Input 和其他系统共享使用。
